...
```{code-block} console
pylith-developer/
├── pylith_installer-5.0.2-0 # source code for the installer
...
tar -xf pylith_installer-5.0.2-0.tar.gz
```
...
$PYLITH_DIR/pylith_installer-5.0.2-0/configure \
    --enable-developer \
    --with-debugging \
...
mkdir $PYLITH_DIR/build-debug
cd $PYLITH_DIR/build-debug
${HOME}/src/pylith/pylith_installer-5.0.2-0/configure  \
    --enable-developer \
    --with-debugging \
    --with-pylith-git=$PYLITH_BRANCH \
    --with-pylith-repo=$PYLITH_REPO \
